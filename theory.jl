...
# ╔═╡ 3664fc00-a642-11f1-a886-4ff811741083
begin
	using LinearAlgebra
	using StaticArrays
	using Rotations
...
# ╔═╡ 366500b0-a642-11f1-b134-0d5a388b3b53
md"""
## Rigid Body Dynamics: Summary of Equations

**Definitions**

- ``X`` — center-of-mass position (world frame)
- ``R \in SO(3)`` — orientation (body-frame → world-frame rotation)
- ``q`` — unit-quaternion representation of ``R``
- ``M`` — total mass
- ``P = M\dot X`` — linear momentum
- ``\rho_i`` — constant body-frame offset of particle ``i`` from the center of mass
- ``I_{\text{body}}`` — inertia tensor (body-fixed, constant, symmetric, positive definite)
- ``\omega`` — angular velocity, body frame
- ``[\omega]_\times`` — skew-symmetric matrix form of ``\omega``, defined by ``[\omega]_\times v = \omega\times v``
- ``L`` — angular momentum about the center of mass (world frame)
- ``F`` — net external force (world frame)
- ``\tau`` — net external torque about the center of mass (world frame)
- ``\tau_{\text{body}} = R^T\tau`` — that torque, expressed in body-frame coordinates

**Configuration.** A rigid body's state is ``(X, R) \in \mathbb{R}^3 \times SO(3)``.
Both translational and rotational equations are anchored at the center of
mass, which is the unique point where they decouple cleanly.
...
**Inertia tensor** (constant, body-fixed):
```math
I_{\text{body}} = \sum_i m_i\left(|\rho_i|^2 I - \rho_i\rho_i^T\right)
```
diagonal in the principal-axis frame.

**Angular momentum** (about the COM, world frame):
```math
L = R\, I_{\text{body}}\, \omega
```

**Equations of motion** (COM-anchored, hence uncoupled from each other):
```math
M\ddot X = F, \qquad \dot L = \tau
```
and, expanding ``L``'s definition and using the kinematics identity,
**Euler's equations** for ``\omega``:
```math
I_{\text{body}}\,\dot\omega = \tau_{\text{body}} - \omega\times(I_{\text{body}}\,\omega)
```

**State for simulation:** ``(X, q, P, L)``.

Velocities are derived quantities, not part of the state:
```math
\dot X = P/M, \qquad \omega_{\text{body}} = I_{\text{body}}^{-1}(R^T L)
```

**Integrator step, per ``\Delta t``:**
```math
P \leftarrow P + \Delta t\, F
```
```math
...
```
```math
q \leftarrow q \otimes \exp\!\left(\tfrac{1}{2}\,\omega_{\text{body}}\,\Delta t\right)
```
the last update via the quaternion exponential map, exact on the unit
quaternion manifold regardless of step size.
"""

# ╔═╡ ee34a4e1-0202-4905-afd0-503f3163c06b
md"""
## General Framework
"""

# ╔═╡ 84da25c7-cae6-45e2-8ec4-78d426b7e3a2
begin
    struct RigidBodyParams
          mass::Float64
          I_body::SMatrix{3,3,Float64,9}
          I_body_inv::SMatrix{3,3,Float64,9}
    end
    RigidBodyParams(mass, I_body) = RigidBodyParams(mass, I_body, inv(I_body))
end

# ╔═╡ a11d2b3a-a81e-4d1f-914c-4d37bc9addee
mutable struct RigidBodyState
      X::SVector{3,Float64}       # COM position, world frame
      q::QuatRotation{Float64}    # orientation
      P::SVector{3,Float64}       # linear momentum, world frame
      L::SVector{3,Float64}       # angular momentum about the COM, world frame
end

# ╔═╡ f7fdfe57-b235-4f1c-af57-79dfe2c91586
function body_angular_velocity(state::RigidBodyState, params::RigidBodyParams)
      R = RotMatrix(state.q)
      params.I_body_inv * (R' * state.L)
end

# ╔═╡ 30e0145c-bdcf-47cf-ade6-74af7c070827
function step!(state::RigidBodyState, params::RigidBodyParams, F::SVector{3,Float64}, τ::SVector{3,Float64}, dt::Float64)
      # momenta and position: exact vector-space update
      state.P += dt * F
      state.L += dt * τ
      state.X += dt * (state.P / params.mass)

      # orientation: exponential map, ω frozen at its current value
      ω_body = body_angular_velocity(state, params)
      θ = norm(ω_body) * dt
      if θ > 0
              axis = ω_body / norm(ω_body)
              Δq = QuatRotation(AngleAxis(θ, axis...))
              state.q = state.q * Δq
      end
      return state
end

# ╔═╡ c99c9f2a-9436-4992-9692-61374916ef2b
md"""
## Example: Torque-Free Tumbling Box
"""

# ╔═╡ 5546f2b3-edc8-4053-8301-b2be1e4914c7
function box_inertia(mass, a, b, c)
      I1 = mass/12 * (b^2 + c^2)
      I2 = mass/12 * (a^2 + c^2)
      I3 = mass/12 * (a^2 + b^2)
      SMatrix{3,3,Float64,9}(Diagonal([I1, I2, I3]))
end

# ╔═╡ 43288e82-f396-48c1-8c82-f8f357936ac1
 box_inertia(1.0, 1.0, 2.0, 3.0)

# ╔═╡ f10d4660-f37c-4ba5-ae03-b06ad17af4e9
begin
      box_params = RigidBodyParams(1.0, box_inertia(1.0, 1.0, 2.0, 3.0))
      ω0_body = SVector(0.01, 5.0, 0.01)   # mostly axis 2 — check which is intermediate below
      q0 = QuatRotation(1.0, 0.0, 0.0, 0.0)
      L0 = q0 * (box_params.I_body * ω0_body)
      box_state = RigidBodyState(SVector(0.0,0,0), q0, SVector(0.0,0,0), L0)
      box_params.I_body
end

# ╔═╡ 4fecc811-8df9-4599-8ab0-00bdea1036e6
begin
      dt = 0.01
      nsteps = 2000
      trajectory = Vector{RigidBodyState}(undef, nsteps)
      s = deepcopy(box_state)
      for i in 1:nsteps
              step!(s, box_params, SVector(0.0,0,0), SVector(0.0,0,0), dt)
              trajectory[i] = deepcopy(s)
      end
      trajectory
end

# ╔═╡ e85012da-0996-4dd0-a2c4-04a6bb673c13
md"""
## Visualization: The Torque-Free Tumbling Box
"""

# ╔═╡ 44c66a79-cb7e-47cb-9647-3d0debcca86f
to_makie_quat(q::QuatRotation) = Makie.Quaternion(q.q.v1, q.q.v2, q.q.v3, q.q.s)

# ╔═╡ 2b712bfd-f4b6-4201-82e5-d5af23709cef
begin
      fig = Figure(size = (600, 450))
      ax = LScene(fig[1, 1], show_axis = true)
      box_geom = GeometryBasics.Rect3f(Point3f(-0.5, -1.0, -1.5), Vec3f(1.0, 2.0, 3.0))
      box_plot = mesh!(ax, box_geom, color = :steelblue)
      fig
end

# ╔═╡ c18d1106-09f7-4231-85f4-bf967df75335
@bind frame Slider(1:nsteps, default=1, show_value=true)

# ╔═╡ 20bb360a-21e7-4ae6-9f2b-ec74c3feac22
Makie.rotate!(box_plot, to_makie_quat(trajectory[frame].q))

# ╔═╡ 039cd0d1-b321-455e-b9f1-e5ae563a6009
# Makie.record(fig, "tumbling_box.mp4", 1:5:nsteps; framerate=60) do i
#       Makie.rotate!(box_plot, to_makie_quat(trajectory[i].q))
# end
...
manifest_format = "2.0"
project_hash = "59cf9aaf025c2689695a34f250f357901dcfd4c1"
